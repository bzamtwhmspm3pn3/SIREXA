...
const CadastroFornecedor = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        setMensagem({ texto: "✅ Fornecedor cadastrado!", tipo: "sucesso" });
        setRedirecting(true);
        setTimeout(() => {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fade-in">
          <div className="bg-gray-800 rounded-2xl max-w-lg w-full max-h-[80vh] overflow-y-auto shadow-2xl border border-purple-500/30 animate-scale-in">
            <div className="bg-gradient-to-r from-purple-600/20 to-pink-600/20 px-6 py-4 border-b border-gray-700 sticky top-0">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-xl font-bold text-white">Adicionar Contrato</h3>
                </div>
            ...
            <div className="px-6 py-4 border-t border-gray-700 flex justify-end gap-3">
              ...
              <button onClick={adicionarContrato} className="px-4 py-2 rounded-lg bg-purple-600 hover:bg-purple-700">Adicionar</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="bg-gray-800 rounded-2xl border border-gray-700 shadow-xl overflow-hidden">
          <div className="bg-gradient-to-r from-blue-600/20 to-purple-600/20 px-6 py-4 border-b border-gray-700">
            <div className="flex items-center gap-3">
              ...
              <div><h2 className="text-xl font-bold text-white">Cadastrar Fornecedor</h2><p className="text-sm text-gray-400">Preencha os dados do fornecedor</p></div>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-6">
            ...
            <div className="bg-gray-700/30 rounded-xl p-4">
              ...
              {isTecnico() ? (
                ...
                <select className="w-full p-3 rounded-xl bg-gray-700/50 border border-gray-600 text-white focus:border-blue-500" value={formData.empresaId} onChange={(e) => setFormData({...formData, empresaId: e.target.value})} required>
            ...
            {/* Resto do formulário igual ao original... */}
            <div className="bg-gray-700/30 rounded-xl p-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="md:col-span-2">
                  ...
                  <input type="text" className="w-full p-3 rounded-xl bg-gray-700/50 border border-gray-600 text-white" value={formData.nome} onChange={(e) => setFormData({...formData, nome: e.target.value})} required />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-2">NIF *</label>
                  <input type="text" className="w-full p-3 rounded-xl bg-gray-700/50 border border-gray-600 text-white" value={formData.nif} onChange={(e) => setFormData({...formData, nif: e.target.value})} required />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-2">Tipo de Serviço</label>
                  <input type="text" className="w-full p-3 rounded-xl bg-gray-700/50 border border-gray-600 text-white" placeholder="Ex: Informática, Construção, Limpeza" value={formData.tipoServico} onChange={(e) => setFormData({...formData, tipoServico: e.target.value})} />
                </div>
              </div>
            </div>
            ...
            <div className="bg-gray-700/30 rounded-xl p-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div><label className="block text-sm font-medium text-gray-300 mb-2">Pessoa de Contacto</label><input type="text" className="w-full p-3 rounded-xl bg-gray-700/50 border border-gray-600 text-white" placeholder="Nome do interlocutor" value={formData.contato} onChange={(e) => setFormData({...formData, contato: e.target.value})} /></div>
            ...
            <div className="bg-gray-700/30 rounded-xl p-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="flex items-center gap-4">
                  <label className="flex items-center gap-2"><input type="checkbox" checked={formData.fiscal.suportaIVA} onChange={(e) => setFormData({...formData, fiscal: {...formData.fiscal, suportaIVA: e.target.checked}})} className="w-4 h-4 text-blue-600 rounded" /><span className="text-gray-300">Suporta IVA</span></label>
                  <label className="flex items-center gap-2"><input type="checkbox" checked={formData.fiscal.retencaoFonte} onChange={(e) => setFormData({...formData, fiscal: {...formData.fiscal, retencaoFonte: e.target.checked}})} className="w-4 h-4 text-blue-600 rounded" /><span className="text-gray-300">Retenção na Fonte</span></label>
                </div>
            ...
            <div className="bg-gray-700/30 rounded-xl p-4">
              ...
              {contratos.length > 0 && (<div className="space-y-2 mb-4">{contratos.map((contrato, index) => (<div key={index} className="flex items-center justify-between p-3 bg-gray-700/30 rounded-lg"><div><p className="text-white font-medium">{formatarMoeda(contrato.valor)} Kz</p><p className="text-xs text-gray-400">{contrato.dataInicio ? new Date(contrato.dataInicio).toLocaleDateString('pt-PT') : '—'} - {contrato.dataFim ? new Date(contrato.dataFim).toLocaleDateString('pt-PT') : '—'}</p><p className="text-xs text-gray-400">{contrato.modalidadePagamento}</p></div><button type="button" onClick={() => removerContrato(index)} className="p-1 text-red-400 hover:text-red-300"><X className="w-5 h-5" /></button></div>))}</div>)}
              <button type="button" onClick={() => setShowContratoModal(true)} className="w-full py-2 border-2 border-dashed border-gray-600 rounded-xl text-gray-400 hover:text-blue-400 hover:border-blue-500 transition-all duration-200 flex items-center justify-center gap-2"><Plus size={18} /> Adicionar Contrato</button>
            </div>
            ...
            <div className="flex gap-3 pt-4">
              <button type="submit" disabled={loading || redirecting} className="flex-1 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 py-3 rounded-xl transition-all duration-300 disabled:opacity-50 shadow-lg hover:shadow-blue-500/25 flex items-center justify-center gap-2">{loading ? <><Loader2 className="w-5 h-5 animate-spin" /> Cadastrando...</> : <><Save className="w-5 h-5" /> Cadastrar Fornecedor</>}</button>
              <button type="button" onClick={() => navigate("/fornecedores")} className="px-6 py-3 bg-gray-700 hover:bg-gray-600 rounded-xl transition-all duration-200 flex items-center gap-2"><ArrowLeft className="w-5 h-5" /> Cancelar</button>
            </div>
          </form>
        </div>
      </div>
